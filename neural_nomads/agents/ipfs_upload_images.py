...
cids = json.loads(TRACKING.read_text()) if TRACKING.exists() else {}
images = sorted([f for f in IMAGES.iterdir() if f.suffix == ".png" and not f.name.startswith(".")])
print(f"Uploading {len(images)} images to IPFS...\n")
...
for i, img in enumerate(images, 1):
    ...
    with open(img, "rb") as f:
        r = requests.post(ENDPOINT, files={"file": (img.name, f, "image/png")}, data={"pinataMetadata": json.dumps({"name": img.name})}, headers=HEADERS)
    if r.status_code == 200:
        cid = r.json()["IpfsHash"]
        cids[img.name] = cid
        TRACKING.write_text(json.dumps(cids, indent=2))
        print(f"  [{i:02d}] OK  {img.name} -> ipfs://{cid}")
    else:
