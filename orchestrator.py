...
def post_farcaster():
    ...
    r = subprocess.run(['python3','agents/farcaster_agent.py'], cwd=nn, capture_output=True, text=True)
    log(r.stdout.strip() or r.stderr.strip())

def build_and_deploy():
    log('Building website...')
    r = subprocess.run(['python3','agents/build_site.py'], cwd=nn, capture_output=True, text=True)
    log(r.stdout.strip() or r.stderr.strip())
    log('Deploying to Vercel...')
    r = subprocess.run(['vercel','--yes','--prod'], cwd=site, capture_output=True, text=True)
    log(r.stdout.strip() or r.stderr.strip())
